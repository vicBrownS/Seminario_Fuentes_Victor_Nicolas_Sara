...
En este panel se renderizará el output que contiene la gráfica, para ello se utiliza la función `plotOutput()` cuyo unico parámetro es el outputid del gráfico(#TODO link al server) a representar, además de varios widgets que ayudarán a la representación y que permiten guardar la imagen de la gráfica.
...
![](Input/Imagenes Documentacion/panelplot.png)
...
-Simplificar eje x (id: simpx): esta opción modifica los niveles de la variable categorica que hay en el eje x y los sustituye por valores numericos desde el 1 hasta el número de niveles que haya. Esta opción se utiliza cuando las categorias son tan largas que no se leen en el gráfico. Su implementación en el server es aquí(#TODO)

-Simplificar eje y (id: simpy): Similar a simpx, pero en el eje y. Su implementación en el server es aquí(#TODO)
...
![](Input/Imagenes Documentacion/Widgets/save.png)

###### Nombre del Archivo

**INPUTID: filename**

[TextInput](https://shiny.posit.co/r/reference/shiny/latest/textinput) que permite al usuario determinar el nombre del archivo PNG en el que se guardará la foto de la gráfica, este TextInput tiene un valor inicial predeterminado que se va actualizando para evitar duplicados en el caso de que no se escoja un nombre manualmente. Este valor se actualiza en el server con la función [`updateTextInput()`](https://shiny.posit.co/r/reference/shiny/latest/updatetextinput) aquí (#TODO)
...
![](Input/Imagenes Documentacion/Widgets/filename.png)

##### Panel Resumen

En este panel se renderizará el output que contiene el resumen de los datos, para ello se utiliza la función [`verbatimTextOutput()`](https://shiny.posit.co/r/reference/shiny/latest/textoutput) cuyo unico parámetro es el outputid del resumen(#TODO link al server).
...
![](Input/Imagenes Documentacion/panelresumen.png)

##### Panel data

En este panel se renderizará el output que contiene la tabla con los datos que se representan, la tabla con los datos se genera en el server aquí(#TODO) y el output se genera aquí(#TODO link al server).
...
En este panel se renderizará la relación entre los niveles de las variables que se representan y sus niveles una vez aplicada la simplificación utilizando las opciones de simpx y simpy ([Opciones de vista]). El texto con esta información se renderiza en el server aquí (#TODO).
...
![](Input/Imagenes Documentacion/panellevels.png){width="467"}

##### Panel guardados (#TODO)
