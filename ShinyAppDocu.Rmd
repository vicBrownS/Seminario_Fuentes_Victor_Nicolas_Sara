...
En este panel se podrá extraer ciertos valores estadísticos como la media, min, max o mediana del dataset que el usuario eliga, para ello hay 4 widgets que permiten al usuario filtrar el tipo de información que quiere sacar. El output se genera en la parte del server aquí([Output\$gotData])
...
-   sexo_representar: sexo que se quiere representar (ver más en [Sexo a Representar (SelectInput)](#sexo-a-representar-selectinput))
...
También se añade a este esqueleto el label de los ejes, la función `theme()` que permite modificar el estilo, tamaño y color de distintas partes de la gráfica y el título de la gráfica formado por el nombre de los datasets a representar.
...
La tercera y última parte de la expresión filtra y devuelve el gráfico en función del tipo elegido([Tipo de gráfico (SelectInput)](#tipo-de-gráfico-selectinput)). Dependiendo del tipo lleva a cabo operaciones distintas:
...
![](Input/Imagenes Documentacion/ejemplomapping.png)
...
![](Input/Imagenes Documentacion/ejemplo_geom_density.png)
...
![](Input/Imagenes Documentacion/ejemplo_geom_histogram.png)
...
![](Input/Imagenes Documentacion/ejemplo_geom_jitter.png)

### Geom_Boxplot {data-link="Mapping"}

La función `geom_boxplot()` es la proporcionada por **ggplot2** para representar los valores indicados en la funcion `ggplot()` como un diagrama de caja. La caja represents
