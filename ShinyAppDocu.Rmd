...
[TextInput](https://shiny.posit.co/r/reference/shiny/latest/textinput) que permite al usuario determinar el nombre del archivo PNG en el que se guardará la foto de la gráfica, este TextInput tiene un valor inicial predeterminado que se va actualizando para evitar duplicados en el caso de que no se escoja un nombre manualmente. Este valor se actualiza en el server con la función [`updateTextInput()`](https://shiny.posit.co/r/reference/shiny/latest/updatetextinput) aquí ([Segundo observador (save)])
...
En este panel se renderizarán las imágenes que el usuario elija, las imágenes se podrán escoger de las que haya en la carpeta `Output\OutputPlot`, un máximo de 4. Estas imágenes se representarán en el orden elegido. El output se genera en el server aquí (#TODO)
...
En este panel se podrá extraer ciertos valores estadísticos como la media, min, max o mediana del dataset que el usuario eliga, para ello hay 4 widgets que permiten al usuario filtrar el tipo de información que quiere sacar. El output se genera en la parte del server aquí(#TODO)
...
-   [Media](https://es.wikipedia.org/wiki/Media_aritm%C3%A9tica), min, max, [mediana](https://es.wikipedia.org/wiki/Mediana_(estad%C3%ADstica))
        ...
        #DEMÁS PANELES
            tabPanel("GetData",
                 column(4, 
                  ...
                  choices = c("media","min","max","mediana"), multiple = T))
...
![](Input/Imagenes Documentacion/Widgets/wanteddata.png)
...
![](Input/Imagenes Documentacion/Widgets/wanteddataset.png)
